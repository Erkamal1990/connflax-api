...
class Admin extends CI_Controller {
	...
	{
		$page_data['page_name']		=	'categories_list';
	...
	function genre_create()
	{
		if (isset($_POST) && !empty($_POST))
		{
			...
			$this->session->set_flashdata('success_genre', 'Genre has been created successfully.');
	...
	function genre_edit($genre_id = '')
	{
		if (isset($_POST) && !empty($_POST))
		{
			...
			$this->session->set_flashdata('success_genre', 'Genre has been updated successfully.');
			redirect(base_url().'index.php?admin/genre_list' , 'refresh');
		}
	...
	function genre_delete($genre_id = '')
	{
		...
		$this->session->set_flashdata('success_genre', 'Genre has been deleted successfully.');
		redirect(base_url().'index.php?admin/genre_list' , 'refresh');
	}
	...
	function subtitle($param1 = '')
	{
		...
		$page_data['page_title']	=	'Manage subtitle : '.$this->db->get_where('movie', array('video_id' => $param1))->row('title');
		$this->load->view('backend/index', $page_data);
	}
	...
	function add_subtitle($param1 = '')
	{
		if (isset($_POST) && !empty($_POST)){
			...
			$subtitle = $this->db->get_where('subtitle', array('video_id' => $param1, 'language' => $language))->row_array();
	...
	function edit_subtitle($param1 = '', $param2 = '')
	{
		if (isset($_POST) && !empty($_POST)){
			$language = $this->input->post('language');
			$subtitle = $this->db->get_where('subtitle', array('video_id' => $param2, 'language' => $language))->row_array();
			if($subtitle['language'] != $language){
	...
	// EDIT A MOVIE
	function movie_edit($video_id = '')
	{
		if (isset($_POST) && !empty($_POST))
		{
			$this->crud_model->update_movie($video_id);
			redirect(base_url().'index.php?admin/movie_list' , 'refresh');
		}
	...
	// DELETE A MOVIE
	function movie_delete($video_id = '')
	{
		$this->db->delete('movie',  array('video_id' => $video_id));
		redirect(base_url().'index.php?admin/movie_list' , 'refresh');
	}
	...
	// EDIT A SERIES
	function series_edit($video_id = '')
	{
		if (isset($_POST) && !empty($_POST))
		{
			$this->crud_model->update_series($video_id);
			redirect(base_url().'index.php?admin/series_edit/'.$video_id , 'refresh');
		}
		$page_data['video_id']		=	$video_id;
	...
	// DELETE A SERIES
	function series_delete($video_id = '')
	{
		$this->db->delete('series',  array('video_id' => $video_id));
		redirect(base_url().'index.php?admin/series_list' , 'refresh');
	}

	// CREATE A NEW SEASON
	function season_create($video_id = '')
	{
	...
	// EDIT A SEASON
	function season_edit($video_id = '', $season_id = '')
	{
		if (isset($_POST) && !empty($_POST))
		{
			$data['title']			=	$this->input->post('title');
			$this->db->update('series', $data,  array('video_id' => $video_id));
			redirect(base_url().'index.php?admin/series_edit/'.$video_id , 'refresh');
		}
		$series_name				=	$this->db->get_where('series', array('video_id'=>$video_id))->row()->title;
	...
	// DELETE A SEASON
	function season_delete($video_id = '', $season_id = '')
	{
	...
	function episode_create($video_id = '', $season_id = '')
	{
		if (isset($_POST) && !empty($_POST))
		{
			$data['title']			=	$this->input->post('title');
			...
			if(!empty($_FILES['series_episode'])){
			   move_uploaded_file($_FILES['series_episode']['tmp_name'], 'assets/global/series_episodes/' . $episode_id . '.mp4');
			   $trailer_url = base_url().'assets/global/series_episodes/'.$episode_id.'.mp4';
			   $this->db->where("episode_id",$episode_id);
			   $this->db->update('episode',array('url'=>$trailer_url));
			}else{
				$trailer_url = '';
			}
			redirect(base_url().'index.php?admin/season_edit/'.$video_id.'/'.$season_id , 'refresh');
		}
	}

	// CREATE A NEW EPISODE
	function episode_edit($video_id = '', $season_id = '', $episode_id = '')
	{
		if (isset($_POST) && !empty($_POST))
		{
			$data['title']			=	$this->input->post('title');
	...
	// DELETE AN EPISODE
	function episode_delete($video_id = '', $season_id = '', $episode_id = '')
	{
	...
	function actor_create()
	{
		if (isset($_POST) && !empty($_POST))
		{
			...
			$this->session->set_flashdata('success_actor', 'Actor has been created successfully.');
	...
	function actor_edit($actor_id = '')
	{
		if (isset($_POST) && !empty($_POST))
		{
			...
			$this->session->set_flashdata('success_actor', 'Actor has been updated successfully.');
			redirect(base_url().'index.php?admin/actor_list' , 'refresh');
		}
	...
	function category_edit($category_id = '')
	{
		if (isset($_POST) && !empty($_POST))
		{
			$this->crud_model->update_actor($category_id);
			$this->session->set_flashdata('success_actor', 'Category has been updated successfully.');
			redirect(base_url().'index.php?admin/categories_list' , 'refresh');
		}
	...
	function category_create()
	{
		if (isset($_POST) && !empty($_POST))
		{
			...
			$this->session->set_flashdata('success_category', 'Category has been created successfully.');
			redirect(base_url().'index.php?admin/categories_list' , 'refresh');
		}
	...
	function actor_delete($actor_id = '')
	{
		...
		$this->session->set_flashdata('success_actor', 'Actor has been deleted successfully.');
		redirect(base_url().'index.php?admin/actor_list' , 'refresh');
	}
	...
	function category_delete($category_id = '')
	{
		...
		$this->session->set_flashdata('success_category', 'Category has been deleted successfully.');
		redirect(base_url().'index.php?admin/categories_list' , 'refresh');
	}
	...
	function director_create()
	{
		if (isset($_POST) && !empty($_POST))
		{
			...
			$this->session->set_flashdata('success_director', 'Director has been created successfully.');
	...
	function director_edit($director_id = '')
	{
		if (isset($_POST) && !empty($_POST))
		{
			...
			$this->session->set_flashdata('success_director', 'Director has been updated successfully.');
			redirect(base_url().'index.php?admin/director_list' , 'refresh');
		}
	...
	function director_delete($director_id = '')
	{
		...
		$this->session->set_flashdata('success_director', 'Director has been deleted successfully.');
		redirect(base_url().'index.php?admin/director_list' , 'refresh');
	}
	...
	function user_create()
	{
		if (isset($_POST) && !empty($_POST)){
			...
			$this->db->or_where('mobile' , $_POST['mobile']);
			$this->db->from('user');
	        ...
	        if($total_number_of_matching_user == 0) {
				...
				$this->session->set_flashdata('success_user', 'User has been created successfully.');
				redirect(base_url().'index.php?admin/user_list' , 'refresh');
			}else{
				$this->session->set_flashdata('failed_user', 'This email/mobile is associate with another user.');
				redirect(base_url().'index.php?admin/user_create' , 'refresh');
			}
		}
	...
	function user_edit($edit_user_id = '')
	{
		if (isset($_POST) && !empty($_POST))
		{
			...
			$this->session->set_flashdata('success_user', 'User details has been updated successfully.');			
			redirect(base_url().'index.php?admin/user_list' , 'refresh');
		}
	...
	function user_delete($user_id = '')
	{
		...
		$this->session->set_flashdata('success_user', 'User has been deleted successfully.');
		redirect(base_url().'index.php?admin/user_list' , 'refresh');
	}
	...
	// WATCH USER PURCHASED STREAM, PAYMENT REPORT
	function user_reports($month = '', $year = '')
	{
		if ($month == '')
			$month	=	date("F");
		if ($year == '')
		...
		$page_data['month']			=	$month;
		$page_data['year']			=	$year;
		$page_data['page_name']		=	'user_reports';
		$page_data['page_title']	=	'Customer Movie/Series Purchase & payment report';
		$this->load->view('backend/index', $page_data);
	}

	// WATCH USER PURCHASED STREAM, PAYMENT REPORT
	function stream_reports($month = '', $year = '')
	{
		if ($month == '')
			$month	=	date("F");
		if ($year == '')
		...
		$page_data['month']			=	$month;
		$page_data['year']			=	$year;
		$page_data['page_name']		=	'stream_reports';
		$page_data['page_title']	=	'Movie/Series Purchase & payment report';
		$this->load->view('backend/index', $page_data);
	}
