...
class Crud_model extends CI_Model {
	...
	*/
	...
	{
		$credential = array('email' => $email,'type' => 1, 'password' => sha1($password));
	...
	function get_user_purchase_report($month, $year)
	{
		$first_day_this_month 			= 	date('01-m-Y' , strtotime($month." ".$year));
		$last_day_this_month  			= 	date('t-m-Y' , strtotime($month." ".$year));
		$timestamp_first_day_this_month	=	strtotime($first_day_this_month);
		...
		$this->db->where('timestamp >' , $timestamp_first_day_this_month);
		$this->db->where('timestamp <' , $timestamp_last_day_this_month);
		$subscriptions = $this->db->get('purchased_stream')->result_array();

		return $subscriptions;
	}

	function get_stream_report($month, $year)
	{
		$first_day_this_month 			= 	date('01-m-Y' , strtotime($month." ".$year));
		$last_day_this_month  			= 	date('t-m-Y' , strtotime($month." ".$year));
		$timestamp_first_day_this_month	=	strtotime($first_day_this_month);
		...
		$this->db->where('timestamp >' , $timestamp_first_day_this_month);
		$this->db->where('timestamp <' , $timestamp_last_day_this_month);
		$this->db->group_by(array("video_id", "series_id"));
		$subscriptions = $this->db->get('purchased_stream')->result_array();
	...
	function create_movie(){
		...
		$data['rating']				=	($this->input->post('rating')) ? $this->input->post('rating') : "";
		$data['genre_id']			=	($this->input->post('genre_id')) ? $this->input->post('genre_id') : "";
		$data['featured']			=	($this->input->post('featured')) ? $this->input->post('featured') : "";
		$data['url']				=	($this->input->post('url')) ? $this->input->post('url') : "";
		$data['trailer_url']  		=	($this->input->post('trailer_url')) ? $this->input->post('trailer_url') : "";
		$data['director']			=	($this->input->post('director')) ? $this->input->post('director') : "";
		$data['created_timestamp']	=	time();
		$data['modefied_timestamp']	=	time();
		...
		move_uploaded_file($_FILES['thumb']['tmp_name'], 'assets/global/movie_thumb/' . $video_id . '.jpg');
		move_uploaded_file($_FILES['poster']['tmp_name'], 'assets/global/movie_poster/' . $video_id . '.jpg');
		

		if(!empty($_FILES['video']) && $data['url'] == ""){
			move_uploaded_file($_FILES['video']['tmp_name'], 'assets/global/videos/' . $video_id . '.mp4');
			$video_url = base_url().'assets/global/videos/'.$video_id.'.mp4';
			$this->db->where("video_id",$video_id);
			$this->db->update('movie',array('url'=>$video_url));
		}else{
			$video_url = '';
		}

		if(!empty($_FILES['trailer']) && $data['trailer_url'] == ""){
			$trailer_url = base_url().'assets/global/movies_trailer/'.$video_id.'.mp4';
			move_uploaded_file($_FILES['trailer']['tmp_name'], 'assets/global/movies_trailer/' . $video_id . '.mp4');
			$this->db->where("video_id",$video_id);
			$this->db->update('movie',array('trailer_url'=>$trailer_url));
		}else{
			$trailer_url = '';
		}
	}
	...
	function update_movie($video_id = ''){
		...
		move_uploaded_file($_FILES['thumb']['tmp_name'], 'assets/global/movie_thumb/' . $video_id . '.jpg');
		move_uploaded_file($_FILES['poster']['tmp_name'], 'assets/global/movie_poster/' . $video_id . '.jpg');
		//move_uploaded_file($_FILES['vtt_file']['tmp_name'], 'assets/global/movie_caption/' . $video_id . '.vtt');
	...
	function create_series(){
		...
		$data['genre_id']			=	($this->input->post('genre_id')) ? $this->input->post('genre_id') : "";
		$data['director']			=	($this->input->post('director')) ? $this->input->post('director') : "";
		$actors						=	($this->input->post('actors')) ? $this->input->post('actors') : "";
		$actor_entries				=	array();
		$number_of_entries			=	sizeof($actors);
		...
		if(!empty($_FILES['trailer']) && $data['trailer_url'] == ""){
			move_uploaded_file($_FILES['trailer']['tmp_name'], 'assets/global/series_trailer/' . $series_id . '.mp4');
			$trailer_url = base_url().'assets/global/series_trailer/'.$series_id.'.mp4';
			$this->db->where("series_id",$series_id);
			$this->db->update('series',array('trailer_url'=>$trailer_url));
		}else{
			$trailer_url = '';
		}
	...
	function create_category()
	{
		...
		move_uploaded_file($_FILES['thumb']['tmp_name'], 'assets/global/categories/' . $category_id . '.jpg');
	}
	...
	function update_category($category_id = '')
	{
		...
		$this->db->update('categories', $data, array('category_id'=>$category_id));
		move_uploaded_file($_FILES['thumb']['tmp_name'], 'assets/global/categories/' . $category_id . '.jpg');
	}
	...
	function create_user(){
		...
		$data['password']			=	sha1($this->input->post('password'));
		$data['type']				=	$this->input->post('user_type');
		$this->db->insert('user', $data);
	}
	...
	{
        if (file_exists('assets/global/categories/' . $id . '.jpg'))
            $image_url = base_url() . 'assets/global/categories/' . $id . '.jpg';
        else
            $image_url = base_url() . 'assets/global/placeholder.jpg';
    ...
    public function get_actor_genre_wise_movies_and_tv_series($actor_id = "", $item = "", $genre_id = "") {
      $item_list = array();
      if ($genre_id != 'all') {
          $this->db->where('genre_id', $genre_id);
      }
